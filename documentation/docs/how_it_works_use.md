...
- code fingerprint can be computed by the user on their own, with the option `--context`. The context file is the tarball containing the context toml and the encrypted code (tarball file) which must be provided by the app owner to the user by their own means. See [the context subcommand](subcommand/context.md#export).
