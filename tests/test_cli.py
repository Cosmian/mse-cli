...
def _capture_logs_init() -> io.StringIO:
...
def _capture_logs(f: io.StringIO) -> str:
...
def _test_deploy(f: io.StringIO, conf: Path) -> Tuple[str, str, str]:
    ...
    run_deploy(Namespace(**{"path": conf}))
...
def _test_mse_ctl(ssl_certificate_origin: SSLCertificateOrigin):
    """Test a complete deployment flow."""
    # Configure log capturing
    f = _capture_logs_init()
...
def test_mse_ctl_with_ssl():
    """Test a complete deployment flow for dev mode."""
    _test_mse_ctl(SSLCertificateOrigin.Owner)


def test_mse_ctl_self_signed():
    """Test a complete deployment flow for dev mode."""
    _test_mse_ctl(SSLCertificateOrigin.Self)


def test_mse_ctl_dev_mode():
    """Test a complete deployment flow for dev mode."""
    _test_mse_ctl(SSLCertificateOrigin.Operator)
